...
local AntiRagdollEnabled, InfiniteJumpEnabled, InstantPromptEnabled = false, false, false
local FPSBoostEnabled, UltraESPEnabled = false, false
local BestBeam = nil
local PerformanceMode = "Ultra"
local PerformanceConfig = {
	DisableParticles = true,
	DisablePostEffects = true,
	DisableTrailsBeams = true,
	DisableShadows = true,
	LowGraphicsQuality = true,
}

local WalkValue, JumpValue, TravelValue, TPValue = 110, 100, 110, 20
local FOVValue = 70
local OriginalFOV = nil
local OriginalWalkSpeed, OriginalJumpPower = nil, nil
local SavedPosition, FloatingSavedPosition = nil, nil
...
local function GetSafeSavedCFrame()
	if IsValidCFrame(SavedPosition) then return SavedPosition end
	SavedPosition = nil
	return nil
end

local Traveling, CancelTravel = false, false
local StepTeleporting, CancelStepTeleport = false, false
...
local function ApplyLoadedNumber(Key, Default, Min, Max)
	local Value = tonumber(LoadedConfig[Key])
	if Value then return math.clamp(Value, Min, Max) end
	return Default
end
...
FOVValue = ApplyLoadedNumber("FOVValue", FOVValue, 30, 120)
...
PerformanceMode = tostring(LoadedConfig.PerformanceMode or "Ultra")
if PerformanceMode ~= "Balanced" and PerformanceMode ~= "Ultra" and PerformanceMode ~= "Extreme" then
...
if type(LoadedConfig.PriorityList) == "table" and #LoadedConfig.PriorityList > 0 then
	for _, name in ipairs(LoadedConfig.PriorityList) do
...
if #PriorityList == 0 then
	for _, name in ipairs(DefaultPriorityList) do
		table.insert(PriorityList, name)
	end
end
...
local function RebuildPriorityMap()
end
...
local ExistingFloating = PlayerGui:FindFirstChild("SnooFloatingTP")
if ExistingFloating then ExistingFloating:Destroy() end
local ExistingStealBar = PlayerGui:FindFirstChild("SnooStealBar")
if ExistingStealBar then ExistingStealBar:Destroy() end
local ExistingBestName = PlayerGui:FindFirstChild("SnooBestName")
if ExistingBestName then ExistingBestName:Destroy() end
...
local Pages = {}
local CurrentPage, CurrentLayout
...
local function CreateTab(Name, Text)
	...
	Button.MouseButton1Click:Connect(function()
		SelectPage(Name)
		for PageName, _ in pairs(Pages) do
...
SelectPage("Movement")
for _, TabName in ipairs({"Movement", "Utility", "Jump", "Credits", "ESP", "Pref", "Config"}) do
...
local function TextBox(Default, Placeholder)
	...
	Object.ZIndex = 5
	Object.Parent = CurrentPage
	Instance.new("UICorner", Object).CornerRadius = UDim.new(0, 10)
...
local StealBarGui, StealBarFill, StealPercentLabel, StealStateLabel = nil, nil, nil, nil
...
local function CreateStealBar()
	...
	local Frame = Instance.new("Frame", StealBarGui)
	...
	StealStateLabel = Instance.new("TextLabel", Frame)
	...
	StealPercentLabel = Instance.new("TextLabel", Frame)
...
local function RemoveStealBar()
	...
	StealBarGui, StealBarFill, StealPercentLabel, StealStateLabel = nil, nil, nil, nil
end
				...
				if Attachment then
					...
					if Distance <= NearestDistance then
						for _, Prompt in ipairs(Attachment:GetChildren()) do
							if Prompt:IsA("ProximityPrompt") and Prompt.Enabled and
								(Prompt.ActionText == "Steal" or string.find(string.lower(Prompt.ActionText), "steal")) then
...
Label("CAMERA FOV")
local FOVBox = TextBox(tostring(FOVValue), "30 - 120")
local FOVButton = Button("  APLICAR FOV")
local ResetFOVButton = Button("  RESET FOV • ORIGINAL")
...
local function ApplyFOV()
	...
	FOVValue = math.clamp(Number, 30, 120)
	if OriginalFOV == nil then OriginalFOV = Camera.FieldOfView end
	Camera.FieldOfView = FOVValue
	FOVBox.Text = tostring(math.floor(FOVValue + 0.5))
	return true
end
...
local function ResetFOV()
	...
	FOVValue = OriginalFOV
	FOVBox.Text = tostring(math.floor(OriginalFOV + 0.5))
	return true
end

FOVButton.MouseButton1Click:Connect(function()
	if ApplyFOV() then
		FOVButton.Text = "  FOV • " .. tostring(math.floor(FOVValue + 0.5))
		if SaveConfig then SaveConfig() end
	else
		FOVButton.Text = "  FOV • VALOR INVÁLIDO"
		task.delay(1.2, function()
			if FOVButton.Parent then FOVButton.Text = "  APLICAR FOV" end
		end)
	end
end)
...
ResetFOVButton.MouseButton1Click:Connect(function()
	if ResetFOV() then
		ResetFOVButton.Text = "  RESET FOV • OK"
		if SaveConfig then SaveConfig() end
		task.delay(1.2, function()
			if ResetFOVButton.Parent then ResetFOVButton.Text = "  RESET FOV • ORIGINAL" end
		end)
	end
end)
...
SpeedButton.MouseButton1Click:Connect(function()
	...
	if not WalkEnabled then
		...
		if Number then WalkValue = math.clamp(Number, 1, 500) end
...
SaveButton.MouseButton1Click:Connect(function()
	...
	task.delay(1.5, function()
		if SaveButton.Parent then SaveButton.Text = "  SALVAR LOCALIZAÇÃO" end
	end)
end)
...
local function StartTravel()
	...
	if not TargetCFrame or not Root or not Humanoid then
		...
		task.delay(1.5, function()
			if TravelButton.Parent then TravelButton.Text = "IR FLUTUANDO ATÉ LOCAL" end
		end)
		return
	end
	...
	local FlightHeight = math.clamp(math.max(StartPosition.Y + 25, Destination.Y + 25), -50000, 50000)
	...
	local function MoveFloating(Target)
		if not Traveling or CancelTravel or not Root or not Root.Parent then return true end
...
TravelButton.MouseButton1Click:Connect(function()
	if Traveling then CancelTravel = true else task.spawn(StartTravel) end
end)
...
local function StartStepTeleport()
	...
	local MaxDuration = math.clamp((Destination - Root.Position).Magnitude / math.max(TPValue, 1) + 10, 10, 180)
...
TPButton.MouseButton1Click:Connect(function()
	if StepTeleporting then CancelStepTeleport = true else task.spawn(StartStepTeleport) end
end)

local SpamTPV1Enabled, SpamTPV2Enabled, BugSpamTPEnabled = false, false, false
local SpamTPV1ButtonSmall, SpamTPV2ButtonSmall, BugSpamTPButtonSmall
local SpamTPV1Connection, SpamTPV2Connection, BugSpamTPConnection = nil, nil, nil
local SpamTPV2Busy, BugSpamTPBusy = false, false
...
local function StopSpamTPV1()
	...
	if SpamTPV1Connection then SpamTPV1Connection:Disconnect() SpamTPV1Connection = nil end
end
...
local function StartSpamTPV1()
	...
	SpamTPV1Connection = RunService.Heartbeat:Connect(function()
		...
		local Now = os.clock()
		if Now - LastSpam < 0.08 then return end
		LastSpam = Now
...
local function SetSpamTPV1(Value)
	...
	if SpamTPV1Enabled then
		if SpamTPV2Enabled then
			...
			if SpamTPV2Connection then SpamTPV2Connection:Disconnect() SpamTPV2Connection = nil end
			SpamTPV2Busy = false
		end
		if BugSpamTPEnabled then
			BugSpamTPEnabled = false
			if BugSpamTPConnection then BugSpamTPConnection:Disconnect() BugSpamTPConnection = nil end
			BugSpamTPBusy = false
		end
		StartSpamTPV1()
	else
	...
	if SpamTPV1Button then SpamTPV1Button.Text = "  SPAM TP V1 • " .. (SpamTPV1Enabled and "ON" or "OFF") end
	if SpamTPV2Button then SpamTPV2Button.Text = "  SPAM TP V2 • " .. (SpamTPV2Enabled and "ON" or "OFF") end
	if BugSpamTPButtonSmall then BugSpamTPButtonSmall.Text = "BUG SPAM TP • " .. (BugSpamTPEnabled and "ON" or "OFF") end
	if SaveConfig then SaveConfig() end
end
...
local function SetSpamTPV2(Value)
	...
	if SpamTPV2Enabled then
		if SpamTPV1Enabled then StopSpamTPV1() end
		if BugSpamTPEnabled then
			BugSpamTPEnabled = false
			if BugSpamTPConnection then BugSpamTPConnection:Disconnect() BugSpamTPConnection = nil end
			BugSpamTPBusy = false
...
local function StopBugSpamTP()
	BugSpamTPEnabled = false
	if BugSpamTPConnection then BugSpamTPConnection:Disconnect() BugSpamTPConnection = nil end
	BugSpamTPBusy = false
end

local function StartBugSpamTP()
	if BugSpamTPConnection then return end
	BugSpamTPConnection = RunService.Heartbeat:Connect(function()
		if not BugSpamTPEnabled or BugSpamTPBusy or not Root or not Root.Parent then return end
		local Target = GetSpamTarget()
		if not IsValidCFrame(Target) then return end
		BugSpamTPBusy = true
		task.spawn(function()
			pcall(function()
				local FrontPos = Target.Position + Target.LookVector * 15
				local FrontCF = CFrame.lookAt(FrontPos, Target.Position)
				Root.AssemblyLinearVelocity = Vector3.zero
				Root.AssemblyAngularVelocity = Vector3.zero
				Root.CFrame = FrontCF
				RunService.Heartbeat:Wait()

				if not BugSpamTPEnabled or not Root or not Root.Parent then
					BugSpamTPBusy = false
					return
				end
				...
				while BugSpamTPEnabled and Root and Root.Parent do
					local Alpha = math.clamp((os.clock() - Started) / Duration, 0, 1)
					local BasePos = StartPos:Lerp(DestPos, Alpha)
					BobPhase = BobPhase + 0.45
					local BobY = math.sin(BobPhase) * 1.8
					local Pos = Vector3.new(BasePos.X, BasePos.Y + BobY, BasePos.Z)
					Root.CFrame = CFrame.lookAt(Pos, DestPos)
					Root.AssemblyLinearVelocity = Vector3.zero
					Root.AssemblyAngularVelocity = Vector3.zero
					if Alpha >= 1 then break end
					RunService.Heartbeat:Wait()
				...
				if BugSpamTPEnabled and Root and Root.Parent then
					Root.CFrame = Target
					Root.AssemblyLinearVelocity = Vector3.zero
					Root.AssemblyAngularVelocity = Vector3.zero
				end
...
local function SetBugSpamTP(Value)
	BugSpamTPEnabled = Value == true
	if BugSpamTPEnabled then
		if SpamTPV1Enabled then StopSpamTPV1() end
		if SpamTPV2Enabled then StopSpamTPV2() end
		StartBugSpamTP()
	else
		StopBugSpamTP()
	end
	if SpamTPV1Button then SpamTPV1Button.Text = "  SPAM TP V1 • " .. (SpamTPV1Enabled and "ON" or "OFF") end
	if SpamTPV2Button then SpamTPV2Button.Text = "  SPAM TP V2 • " .. (SpamTPV2Enabled and "ON" or "OFF") end
	if SpamTPV1ButtonSmall then SpamTPV1ButtonSmall.Text = "SPAM TP V1 • " .. (SpamTPV1Enabled and "ON" or "OFF") end
	if SpamTPV2ButtonSmall then SpamTPV2ButtonSmall.Text = "SPAM TP V2 • " .. (SpamTPV2Enabled and "ON" or "OFF") end
	if BugSpamTPButtonSmall then BugSpamTPButtonSmall.Text = "BUG SPAM TP • " .. (BugSpamTPEnabled and "ON" or "OFF") end
	if SaveConfig then SaveConfig() end
end
...
SpamTPV1Button.MouseButton1Click:Connect(function()
	...
	if not IsValidCFrame(Target) then
		...
		task.delay(1.2, function()
			if SpamTPV1Button.Parent then SpamTPV1Button.Text = "  SPAM TP V1 • " .. (SpamTPV1Enabled and "ON" or "OFF") end
		end)
		return
	end
...
SpamTPV2Button.MouseButton1Click:Connect(function()
	...
	if not IsValidCFrame(Target) then
		...
		task.delay(1.2, function()
			if SpamTPV2Button.Parent then SpamTPV2Button.Text = "  SPAM TP V2 • " .. (SpamTPV2Enabled and "ON" or "OFF") end
		end)
		return
	end
...
local FPSOriginals = {}
local FPSOriginalLighting, FPSOriginalQuality = nil, nil
...
local function ApplyFPSBoost()
	...
	pcall(function()
		...
		if PerformanceMode == "Extreme" then Lighting.FogEnd = math.min(Lighting.FogEnd, 2500)
		elseif PerformanceMode == "Ultra" then Lighting.FogEnd = math.min(Lighting.FogEnd, 5000) end
	end)
	pcall(function()
		local GameSettings = UserSettings():GetService("UserGameSettings")
		FPSOriginalQuality = GameSettings.SavedQualityLevel
		if PerformanceMode ~= "Balanced" then GameSettings.SavedQualityLevel = Enum.SavedQualitySetting.QualityLevel1 end
	end)
	for _, Object in ipairs(workspace:GetDescendants()) do
		if Object:IsA("BasePart") and PerformanceConfig.DisableShadows then
			FPSOriginals[Object] = FPSOriginals[Object] or {CastShadow = Object.CastShadow}
			Object.CastShadow = false
		elseif (Object:IsA("ParticleEmitter") and PerformanceConfig.DisableParticles)
			or ((Object:IsA("Trail") or Object:IsA("Beam")) and Object ~= BestBeam and PerformanceConfig.DisableTrailsBeams)
			or (Object:IsA("PostEffect") and PerformanceConfig.DisablePostEffects) then
			FPSOriginals[Object] = FPSOriginals[Object] or {Enabled = Object.Enabled}
			Object.Enabled = false
		end
	end
	FPSButton.Text = "  FPS BOOST • " .. PerformanceMode:upper()
end
...
local function RemoveFPSBoost()
	...
	for Object, Original in pairs(FPSOriginals) do
		if Object and Object.Parent then
			pcall(function()
				if Original.CastShadow ~= nil then Object.CastShadow = Original.CastShadow end
				if Original.Enabled ~= nil then Object.Enabled = Original.Enabled end
			end)
		end
	end
	...
	pcall(function()
		local GameSettings = UserSettings():GetService("UserGameSettings")
		if FPSOriginalQuality then GameSettings.SavedQualityLevel = FPSOriginalQuality end
	end)
	FPSButton.Text = "  FPS BOOST • OFF"
end

FPSButton.MouseButton1Click:Connect(function()
	if FPSBoostEnabled then RemoveFPSBoost() else ApplyFPSBoost() end
	if SaveConfig then SaveConfig() end
end)
...
local function ConfigurePrompt(Prompt)
	...
	if InstantPromptEnabled then
		if Prompt:GetAttribute("SnooOriginalHold") == nil then
			Prompt:SetAttribute("SnooOriginalHold", Prompt.HoldDuration)
		end
		Prompt.HoldDuration = 0
		Prompt.RequiresLineOfSight = false
	else
		local Original = Prompt:GetAttribute("SnooOriginalHold")
		if typeof(Original) == "number" then Prompt.HoldDuration = Original end
	end
end

local function UpdatePrompts()
	for _, Object in ipairs(workspace:GetDescendants()) do
		if Object:IsA("ProximityPrompt") then ConfigurePrompt(Object) end
	end
end
...
JumpButton.MouseButton1Click:Connect(function()
	...
	if not JumpEnabled then
		...
		if Number then JumpValue = math.clamp(Number, 1, 500) end
		Humanoid.UseJumpPower = true
		Humanoid.JumpPower = JumpValue
		JumpEnabled = true
...
local function StartInfiniteJump()
	if InfiniteJumpRequestConnection then InfiniteJumpRequestConnection:Disconnect() end
...
local function ForceAntiRagdollReset()
	...
	pcall(function()
		...
		for _, Object in ipairs(Character:GetDescendants()) do
			if Object:IsA("Motor6D") or Object:IsA("Constraint") then Object.Enabled = true end
		end
...
RunService.Heartbeat:Connect(function()
	local Now = os.clock()
	if Now - LastMovementUpdate < 0.05 then return end
...
local NoclipConnection = RunService.Stepped:Connect(function()
	...
	elseif next(NoclipOriginals) then
...
Minimize.MouseButton1Click:Connect(function()
	...
	if Minimized then
		...
		Minimize.Text = "+"
		Main:TweenSize(UDim2.fromOffset(200, 52), Enum.EasingDirection.Out, Enum.EasingStyle.Quad, 0.2, true)
	else
		Content.Visible = true
		TabBar.Visible = true
		Minimize.Text = "—"
		Main:TweenSize(UDim2.fromOffset(540, 420), Enum.EasingDirection.Out, Enum.EasingStyle.Quad, 0.2, true)
	end
end)
...
local function TPBest()
	...
	local TargetCFrame = CFrame.new(Target.Part.Position + Vector3.new(0, 4, 0))
...
local FloatingTP = Instance.new("Frame", FloatingTPGui)
FloatingTP.Name = "FloatingTP"
FloatingTP.Size = UDim2.fromOffset(172, 248)
...
Instance.new("UICorner", FloatingTP).CornerRadius = UDim.new(0, 10)
local FloatingStroke = Instance.new("UIStroke", FloatingTP)
FloatingStroke.Color = Color3.fromRGB(155, 70, 255)
FloatingStroke.Thickness = 1.5
FloatingStroke.Transparency = 0.12

local FloatingTitle = Instance.new("TextLabel", FloatingTP)
...
local SaveTPButton = Instance.new("TextButton", FloatingTP)
...
local TPButtonSmall = Instance.new("TextButton", FloatingTP)
...
local TPBestButton = Instance.new("TextButton", FloatingTP)
...
local TravelButtonSmall = Instance.new("TextButton", FloatingTP)
...
SpamTPV1ButtonSmall = Instance.new("TextButton", FloatingTP)
...
SpamTPV2ButtonSmall = Instance.new("TextButton", FloatingTP)
...
BugSpamTPButtonSmall = Instance.new("TextButton", FloatingTP)
BugSpamTPButtonSmall.Size = UDim2.new(1, -12, 0, 28)
BugSpamTPButtonSmall.Position = UDim2.fromOffset(6, 194)
BugSpamTPButtonSmall.BackgroundColor3 = Color3.fromRGB(24, 24, 28)
BugSpamTPButtonSmall.BorderSizePixel = 0
BugSpamTPButtonSmall.Text = "BUG SPAM TP • OFF"
BugSpamTPButtonSmall.TextColor3 = Color3.fromRGB(245, 245, 245)
BugSpamTPButtonSmall.TextSize = 8
BugSpamTPButtonSmall.Font = Enum.Font.GothamBold
BugSpamTPButtonSmall.AutoButtonColor = false

for _, Btn in ipairs({SaveTPButton, TPButtonSmall, TPBestButton, TravelButtonSmall, SpamTPV1ButtonSmall, SpamTPV2ButtonSmall, BugSpamTPButtonSmall}) do
	Instance.new("UICorner", Btn).CornerRadius = UDim.new(0, 8)
	local Stroke = Instance.new("UIStroke", Btn)
	Stroke.Color = Color3.fromRGB(40, 40, 48)
	Stroke.Transparency = 0.1
	Btn.MouseEnter:Connect(function()
		Btn.BackgroundColor3 = Color3.fromRGB(42, 25, 60)
		Stroke.Color = Color3.fromRGB(190, 90, 255)
	end)
...
BugSpamTPEnabled = LoadedConfig.BugSpamTPEnabled == true
UltraESPEnabled = LoadedConfig.UltraESPEnabled == true
if SpamTPV1Enabled and SpamTPV2Enabled then SpamTPV2Enabled = false end
if BugSpamTPEnabled and (SpamTPV1Enabled or SpamTPV2Enabled) then
	SpamTPV1Enabled = false
	SpamTPV2Enabled = false
end
...
SaveConfig = function()
	...
	local Data = {
		WalkValue = WalkValue, JumpValue = JumpValue, TravelValue = TravelValue, TPValue = TPValue, FOVValue = FOVValue,
		StealDuration = StealDuration, StealRadius = StealRadius,
		WalkEnabled = WalkEnabled, JumpEnabled = JumpEnabled, NoclipEnabled = NoclipEnabled,
		AntiRagdollEnabled = AntiRagdollEnabled, InfiniteJumpEnabled = InfiniteJumpEnabled,
		InstantPromptEnabled = InstantPromptEnabled, FPSBoostEnabled = FPSBoostEnabled,
		PerformanceMode = PerformanceMode, AutoStealEnabled = AutoStealEnabled,
		SpamTPV1Enabled = SpamTPV1Enabled, SpamTPV2Enabled = SpamTPV2Enabled,
		BugSpamTPEnabled = BugSpamTPEnabled,
		UltraESPEnabled = UltraESPEnabled, Language = Language,
		PriorityList = PriorityList,
		SavedTP = FloatingSavedPosition and {FloatingSavedPosition:GetComponents()} or nil,
...
SaveTPButton.MouseButton1Click:Connect(function()
	if not Root then
		...
		task.delay(1.2, function() if SaveTPButton.Parent then SaveTPButton.Text = "SALVAR TP" end end)
		return
	end
...
SpamTPV1ButtonSmall.MouseButton1Click:Connect(function()
	...
	if not IsValidCFrame(Target) then
		...
		task.delay(1.2, function()
			if SpamTPV1ButtonSmall.Parent then SpamTPV1ButtonSmall.Text = "SPAM TP V1 • " .. (SpamTPV1Enabled and "ON" or "OFF") end
		end)
		return
	end
	SetSpamTPV1(not SpamTPV1Enabled)
...
SpamTPV2ButtonSmall.MouseButton1Click:Connect(function()
	...
	if not IsValidCFrame(Target) then
		...
		task.delay(1.2, function()
			if SpamTPV2ButtonSmall.Parent then SpamTPV2ButtonSmall.Text = "SPAM TP V2 • " .. (SpamTPV2Enabled and "ON" or "OFF") end
		end)
		return
	end
	SetSpamTPV2(not SpamTPV2Enabled)
...
BugSpamTPButtonSmall.MouseButton1Click:Connect(function()
	local Target = GetSpamTarget()
	if not IsValidCFrame(Target) then
		BugSpamTPButtonSmall.Text = "SEM LOCAL"
		task.delay(1.2, function()
			if BugSpamTPButtonSmall.Parent then BugSpamTPButtonSmall.Text = "BUG SPAM TP • " .. (BugSpamTPEnabled and "ON" or "OFF") end
		end)
		return
	end
	SetBugSpamTP(not BugSpamTPEnabled)
end)
...
TPButtonSmall.MouseButton1Click:Connect(function()
	...
	if not IsValidCFrame(Target) or not Root then
		...
		task.delay(1.2, function() if TPButtonSmall.Parent then TPButtonSmall.Text = "TP" end end)
		return
	end
	Root.CFrame = Target
	Root.AssemblyLinearVelocity = Vector3.zero
...
TPBestButton.MouseButton1Click:Connect(function()
	...
	if Target then
		...
		TPBest()
		local Name = Target.Name or "MELHOR"
...
TravelButtonSmall.MouseButton1Click:Connect(function()
	...
	if not (FloatingSavedPosition or SavedPosition) then
		...
		task.delay(1.2, function()
			if TravelButtonSmall.Parent then TravelButtonSmall.Text = "IR PELO TRAJETO" end
		end)
		return
	end
...
local FloatingDragging, FloatingDragStart, FloatingStartPos = false, nil, nil
local function IsInsideButton(Pos)
	for _, Obj in ipairs(FloatingTP:GetDescendants()) do
		if Obj:IsA("GuiButton") and Obj.Visible then
			local P, S = Obj.AbsolutePosition, Obj.AbsoluteSize
			if Pos.X >= P.X and Pos.X <= P.X + S.X and Pos.Y >= P.Y and Pos.Y <= P.Y + S.Y then return true end
		end
	end
	return false
end
...
FloatingTP.InputBegan:Connect(function(Input)
	if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
		if IsInsideButton(Input.Position) then return end
		FloatingDragging = true
		FloatingDragStart = Input.Position
		FloatingStartPos = FloatingTP.Position
	end
end)
UserInputService.InputChanged:Connect(function(Input)
	...
	if Input.UserInputType == Enum.UserInputType.MouseMovement or Input.UserInputType == Enum.UserInputType.Touch then
		...
		local NewX = math.clamp(FloatingStartPos.X.Offset + Delta.X, 4, math.max(4, Viewport.X - FloatingTP.AbsoluteSize.X - 4))
		local NewY = math.clamp(FloatingStartPos.Y.Offset + Delta.Y, 4, math.max(4, Viewport.Y - FloatingTP.AbsoluteSize.Y - 4))
...
SelectPage("Pref")
Section("PREFERÊNCIA DE BRAINROTS")
Description("Arraste a ordem com os botões. Digite o nome exato para adicionar.")
Description("A lista é salva automaticamente no arquivo JSON.")

local PrefStatus = Label("Lista carregada (" .. #PriorityList .. " itens)")

local AddBox = TextBox("", "Digite o nome exato do Brainrot")
local AddButton = Button("  ADICIONAR NA LISTA")

local PrefListFrame = Instance.new("Frame")
PrefListFrame.Size = UDim2.new(1, -8, 0, 220)
PrefListFrame.BackgroundColor3 = Color3.fromRGB(18, 18, 22)
PrefListFrame.BorderSizePixel = 0
PrefListFrame.ClipsDescendants = true
PrefListFrame.Parent = CurrentPage
Instance.new("UICorner", PrefListFrame).CornerRadius = UDim.new(0, 8)

local PrefScroll = Instance.new("ScrollingFrame", PrefListFrame)
PrefScroll.Size = UDim2.fromScale(1, 1)
PrefScroll.BackgroundTransparency = 1
PrefScroll.BorderSizePixel = 0
PrefScroll.ScrollBarThickness = 4
PrefScroll.ScrollBarImageColor3 = Color3.fromRGB(155, 70, 255)
PrefScroll.CanvasSize = UDim2.new(0, 0, 0, 0)

local PrefLayout = Instance.new("UIListLayout", PrefScroll)
PrefLayout.Padding = UDim.new(0, 3)
PrefLayout.SortOrder = Enum.SortOrder.LayoutOrder

local function RefreshPrefList()
	for _, child in ipairs(PrefScroll:GetChildren()) do
	...
	for i, name in ipairs(PriorityList) do
		...
		row.LayoutOrder = i
		row.Parent = PrefScroll
		Instance.new("UICorner", row).CornerRadius = UDim.new(0, 6)

		local num = Instance.new("TextLabel", row)
		num.Size = UDim2.fromOffset(28, 26)
		num.BackgroundTransparency = 1
		num.Text = tostring(i)
		num.TextColor3 = Color3.fromRGB(190, 90, 255)
		num.TextSize = 11
		...
		local upBtn = MakeSmallBtn("↑", -100)
		local downBtn = MakeSmallBtn("↓", -68)
		local remBtn = MakeSmallBtn("X", -36)
		...
		upBtn.MouseButton1Click:Connect(function()
			if i > 1 then
				...
				RefreshPrefList()
				if SaveConfig then SaveConfig() end
				PrefStatus.Text = "Lista atualizada (" .. #PriorityList .. " itens)"
			end
		end)
		...
		remBtn.MouseButton1Click:Connect(function()
			...
			RefreshPrefList()
			if SaveConfig then SaveConfig() end
			PrefStatus.Text = "Removido. Agora: " .. #PriorityList .. " itens"
		end)
	end

	PrefScroll.CanvasSize = UDim2.fromOffset(0, PrefLayout.AbsoluteContentSize.Y + 8)
	PrefStatus.Text = "Lista carregada (" .. #PriorityList .. " itens)"
end

AddButton.MouseButton1Click:Connect(function()
	local raw = AddBox.Text
	if not raw or raw:match("^%s*$") then
		PrefStatus.Text = "Digite um nome válido"
		return
	end
	local clean = raw:match("^%s*(.-)%s*$")
	local norm = NormalizeName(clean)

	for _, existing in ipairs(PriorityList) do
		if NormalizeName(existing) == norm then
			PrefStatus.Text = "Já existe na lista!"
			return
		end
	end
	...
	RefreshPrefList()
	if SaveConfig then SaveConfig() end
	PrefStatus.Text = "Adicionado: " .. clean
end)

local ResetDefaultBtn = Button("  RESTAURAR LISTA PADRÃO")
ResetDefaultBtn.MouseButton1Click:Connect(function()
	table.clear(PriorityList)
	for _, name in ipairs(DefaultPriorityList) do
		table.insert(PriorityList, name)
	end
	RefreshPrefList()
	if SaveConfig then SaveConfig() end
	PrefStatus.Text = "Lista padrão restaurada"
end)
...
local function ESPGetBrainrotName(Object)
	...
	for _, attr in ipairs({"BrainrotName", "DisplayName", "AnimalName", "ModelName", "Brainrot"}) do
		local v = Object:GetAttribute(attr)
		if typeof(v) == "string" and v ~= "" then table.insert(candidates, v) end
	end
	for _, childName in ipairs({"BrainrotName", "DisplayName", "AnimalName", "ModelName", "Brainrot"}) do
		local child = Object:FindFirstChild(childName, true)
		if child then
			if child:IsA("StringValue") and child.Value ~= "" then table.insert(candidates, child.Value)
			elseif child:IsA("TextLabel") and child.Text ~= "" then table.insert(candidates, child.Text) end
		end
	end
	for _, value in ipairs(candidates) do
		local prio = GetPriorityOf(value)
		if prio then return value, prio end
	end
	return nil, nil
end
...
local function ESPGetPart(Object)
	...
	if Object:IsA("Model") then
		return Object.PrimaryPart or Object:FindFirstChild("HumanoidRootPart", true) or Object:FindFirstChildWhichIsA("BasePart", true)
	end
	return Object:FindFirstChildWhichIsA("BasePart", true)
end

local function ESPIsSlot(Object)
	local n = tonumber(string.match(Object.Name, "^%s*(%d+)%s*$"))
	return n and n >= 1 and n <= 8, n
end

local ActiveNameTags = {}

local function CreateNameTag(part, displayName)
	if not part or not part.Parent then return nil end
	local bill = Instance.new("BillboardGui")
	bill.Name = "SnooBrainrotName"
	bill.Adornee = part
	bill.Size = UDim2.fromOffset(160, 32)
	bill.StudsOffset = Vector3.new(0, 3.4, 0)
	bill.AlwaysOnTop = true
	bill.MaxDistance = 350
	bill.LightInfluence = 0
	bill.Parent = PlayerGui

	local box = Instance.new("Frame", bill)
	box.Size = UDim2.fromScale(1, 1)
	box.BackgroundColor3 = Color3.fromRGB(25, 10, 40)
	...
	label.Text = tostring(displayName or "?")
...
local function ClearAllNameTags()
	for obj, bill in pairs(ActiveNameTags) do
...
local function UpdateAllNameTags()
	...
	for _, obj in ipairs(Plots:GetDescendants()) do
		if ESPIsSlot(obj) then
			local candidates = {obj}
			for _, child in ipairs(obj:GetChildren()) do
				if child:IsA("Model") or child:IsA("BasePart") then table.insert(candidates, child) end
			end
			for _, cand in ipairs(candidates) do
				local name, prio = ESPGetBrainrotName(cand)
				local part = name and ESPGetPart(cand)
				if name and part and part.Parent then
					found[cand] = {Name = name, Part = part}
				end
			end
		end
	end
	for obj, bill in pairs(ActiveNameTags) do
		if not found[obj] or not obj.Parent then
			if bill and bill.Parent then bill:Destroy() end
			ActiveNameTags[obj] = nil
		end
	end
	for obj, data in pairs(found) do
		local existing = ActiveNameTags[obj]
		if existing and existing.Parent then
			if existing.Adornee ~= data.Part then existing.Adornee = data.Part end
		else
			local bill = CreateNameTag(data.Part, data.Name)
			if bill then ActiveNameTags[obj] = bill end
		end
	end
end

local ESPBestCache, ESPBestCacheAt = nil, 0
local ESP_SCAN_INTERVAL = 1.0
...
local function ESPFindBestTarget(Force)
	...
	for _, obj in ipairs(Plots:GetDescendants()) do
		if ESPIsSlot(obj) then
			local name, prio = ESPGetBrainrotName(obj)
			local part = name and ESPGetPart(obj)
			if name and prio and part and prio < BestPriority then
				BestPriority = prio
				Best = {Name = name, Priority = prio, Part = part, Object = obj}
			end
			for _, cand in ipairs(obj:GetChildren()) do
				if cand:IsA("Model") or cand:IsA("BasePart") then
					local cName, cPrio = ESPGetBrainrotName(cand)
					local cPart = cName and ESPGetPart(cand)
					if cName and cPrio and cPart and cPrio < BestPriority then
						BestPriority = cPrio
						Best = {Name = cName, Priority = cPrio, Part = cPart, Object = cand}
					end
				end
			end
		end
...
BestBeam.Name = "SnooBestBeam"
BestBeam.FaceCamera = true
BestBeam.Enabled = false
BestBeam.Width0 = 0.55
BestBeam.Width1 = 0.35
BestBeam.Segments = 36
BestBeam.LightEmission = 1
BestBeam.LightInfluence = 0
BestBeam.Color = ColorSequence.new({
	ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 60, 120)),
	ColorSequenceKeypoint.new(0.5, Color3.fromRGB(255, 30, 80)),
	ColorSequenceKeypoint.new(1, Color3.fromRGB(200, 20, 60))
})
BestBeam.Transparency = NumberSequence.new({
	NumberSequenceKeypoint.new(0, 0.0),
	NumberSequenceKeypoint.new(0.7, 0.05),
	NumberSequenceKeypoint.new(1, 0.2)
})
BestBeam.Parent = workspace

local PlayerAtt, TargetAtt, CurrentBeamPart = nil, nil, nil

local function DestroyBeamAttachments()
	if PlayerAtt then PlayerAtt:Destroy() PlayerAtt = nil end
	if TargetAtt then TargetAtt:Destroy() TargetAtt = nil end
	CurrentBeamPart = nil
	BestBeam.Attachment0 = nil
	BestBeam.Attachment1 = nil
	BestBeam.Enabled = false
	BestBeam.CurveSize0 = 0
	BestBeam.CurveSize1 = 0
end

local function EnsurePlayerAttachment()
	local root = Player.Character and Player.Character:FindFirstChild("HumanoidRootPart")
	if not root then DestroyBeamAttachments() return nil end
	if PlayerAtt and PlayerAtt.Parent == root then return PlayerAtt end
	if PlayerAtt then PlayerAtt:Destroy() end
	PlayerAtt = Instance.new("Attachment")
	PlayerAtt.Name = "SnooBest_Player"
	PlayerAtt.Position = Vector3.new(0, 0.25, 0)
	PlayerAtt.Parent = root
	return PlayerAtt
end

local function EnsureTargetAttachment(part)
	if not part or not part:IsA("BasePart") or not part.Parent then return nil end
	if TargetAtt and TargetAtt.Parent == part then return TargetAtt end
	if TargetAtt then TargetAtt:Destroy() end
	TargetAtt = Instance.new("Attachment")
	TargetAtt.Name = "SnooBest_Target"
	TargetAtt.Position = Vector3.zero
	TargetAtt.Parent = part
	CurrentBeamPart = part
	return TargetAtt
end

local function AttachBestBeam(targetPart)
	local a0 = EnsurePlayerAttachment()
	local a1 = EnsureTargetAttachment(targetPart)
	if not a0 or not a1 then DestroyBeamAttachments() return false end
	BestBeam.Attachment0 = a0
	BestBeam.Attachment1 = a1
	local root = Player.Character and Player.Character:FindFirstChild("HumanoidRootPart")
	if root then
		local heightDiff = targetPart.Position.Y - root.Position.Y
		local curve = math.clamp(heightDiff * 0.09, -14, 20)
		BestBeam.CurveSize0 = curve * 0.5
		BestBeam.CurveSize1 = curve
	else
		BestBeam.CurveSize0 = 0
		BestBeam.CurveSize1 = 0
	end
	BestBeam.Enabled = true
	return true
end

local BestNameBillboard = Instance.new("BillboardGui")
BestNameBillboard.Name = "SnooBestName"
BestNameBillboard.Size = UDim2.fromOffset(190, 38)
BestNameBillboard.StudsOffset = Vector3.new(0, 4.0, 0)
BestNameBillboard.AlwaysOnTop = true
BestNameBillboard.MaxDistance = 400
BestNameBillboard.Enabled = false
BestNameBillboard.Parent = PlayerGui

local BestNameBox = Instance.new("Frame", BestNameBillboard)
BestNameBox.Size = UDim2.fromScale(1, 1)
BestNameBox.BackgroundColor3 = Color3.fromRGB(70, 20, 130)
BestNameBox.BackgroundTransparency = 0.12
BestNameBox.BorderSizePixel = 0
Instance.new("UICorner", BestNameBox).CornerRadius = UDim.new(0, 9)
local BestNameStroke = Instance.new("UIStroke", BestNameBox)
BestNameStroke.Color = Color3.fromRGB(220, 100, 255)
BestNameStroke.Thickness = 2.2

local BestNameLabel = Instance.new("TextLabel", BestNameBox)
BestNameLabel.Size = UDim2.fromScale(1, 1)
BestNameLabel.BackgroundTransparency = 1
BestNameLabel.Text = ""
BestNameLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
BestNameLabel.TextStrokeTransparency = 0.3
BestNameLabel.TextSize = 14
BestNameLabel.Font = Enum.Font.GothamBlack
...
local function ESPUpdate()
	...
	if not target or not target.Object or not target.Object.Parent or not target.Part or not target.Part.Parent then
	...
	if CurrentBeamPart ~= target.Part or not PlayerAtt or PlayerAtt.Parent ~= root or not TargetAtt or TargetAtt.Parent ~= target.Part then
		AttachBestBeam(target.Part)
	else
		local heightDiff = target.Part.Position.Y - root.Position.Y
		local curve = math.clamp(heightDiff * 0.09, -14, 20)
		BestBeam.CurveSize0 = curve * 0.5
		BestBeam.CurveSize1 = curve
		BestBeam.Enabled = true
	end
	BestNameBillboard.Adornee = target.Part
	BestNameLabel.Text = "★ " .. tostring(target.Name or "MELHOR")
	BestNameBillboard.Enabled = true
end
...
local function ESPSetEnabled(state)
	...
	if UltraESPEnabled then
		...
		ClearAllNameTags()
		if not BestLineEnabled then
			CurrentBestTarget = nil
			DestroyBeamAttachments()
			BestNameBillboard.Enabled = false
		end
	end
	if SaveConfig then SaveConfig() end
...
RunService.Heartbeat:Connect(function()
	...
	local now = os.clock()
	if now - lastVisualUpdate < 0.03 then return end
...
task.spawn(function()
	while Gui and Gui.Parent do
		task.wait(0.7)
		if UltraESPEnabled then UpdateAllNameTags() end
	end
end)
...
task.spawn(function()
	while Gui and Gui.Parent do
		...
		if UltraESPEnabled or BestLineEnabled then
			local found = ESPFindBestTarget(false)
			if found then
				CurrentBestTarget = found
				ESPCurrentTarget = found
			end
...
SelectPage("ESP")
Section("ESP ULTRA V1.1")
Description("Linha mais visível + Nome em TODOS os Brainrots da lista")
Description("Use a aba PREF para editar a ordem de preferência")
...
SelectPage("Config")
Section("PERFORMANCE")
Description("Modo Ultra reduz sombras, partículas, efeitos e qualidade.")
local PerformanceButton = Button("  FPS BOOST " .. PerformanceMode:upper())
PerformanceButton.MouseButton1Click:Connect(function()
	local Modes = {"Balanced", "Ultra", "Extreme"}
	local Index = table.find(Modes, PerformanceMode) or 2
	PerformanceMode = Modes[Index % #Modes + 1]
	if FPSBoostEnabled then
		RemoveFPSBoost()
		ApplyFPSBoost()
	else
		PerformanceButton.Text = "  FPS BOOST " .. PerformanceMode:upper()
	end
	SaveConfig()
end)

local FPSApplyButton = Button("  ATIVAR FPS BOOST AGORA")
FPSApplyButton.MouseButton1Click:Connect(function()
	if FPSBoostEnabled then RemoveFPSBoost() else ApplyFPSBoost() end
	PerformanceButton.Text = "  FPS BOOST " .. PerformanceMode:upper()
	FPSApplyButton.Text = FPSBoostEnabled and "  FPS BOOST ATIVO" or "  ATIVAR FPS BOOST AGORA"
	SaveConfig()
end)
...
if InitialCamera then OriginalFOV = InitialCamera.FieldOfView end
FOVBox.Text = tostring(math.floor(FOVValue + 0.5))
JumpButton.Text = "  JUMP POWER • " .. (JumpEnabled and "ON" or "OFF")
NoclipButton.Text = "  NOCLIP • " .. (NoclipEnabled and "ON" or "OFF")
AntiRagdollButton.Text = "  ANTI-RAGDOLL • " .. (AntiRagdollEnabled and "ON" or "OFF")
PromptButton.Text = "  INSTANT PROMPT • " .. (InstantPromptEnabled and "ON" or "OFF")
FPSButton.Text = "  FPS BOOST • " .. (FPSBoostEnabled and PerformanceMode:upper() or "OFF")
AutoStealButton.Text = "  AUTO STEAL • " .. (AutoStealEnabled and "ON" or "OFF")
InfiniteButton.Text = "  INFINITY JUMP • " .. (InfiniteJumpEnabled and "ON" or "OFF")
...
BugSpamTPButtonSmall.Text = "BUG SPAM TP • " .. (BugSpamTPEnabled and "ON" or "OFF")
...
if BugSpamTPEnabled then StartBugSpamTP() end
...
print("Snoo Hub V1.1 carregado com sucesso!")
